...
List model(List parscpp) { 
  ...
  const std::vector<double> Sa0( parscpp["Sa0"]);  // Susceptible
  const std::vector<double> E1a0(parscpp["E1a0"]); // Exposed TODO: update E1, E2
  const std::vector<double> E2a0(parscpp["E1a0"]); // Exposed
  const std::vector<double> U1a0(parscpp["U1a0"]); // Sub-clinical cases TODO: update U1, U2
  const std::vector<double> U2a0(parscpp["U2a0"]); // Sub-clinical cases
  const std::vector<double> I1a0(parscpp["I1a0"]); // Pre-clinical cases TODO: update I1, I2
  const std::vector<double> I2a0(parscpp["I2a0"]); // Clinical cases
  const std::vector<double> Ra0(parscpp["Ra0"]);   // Recovered
  const std::vector<double> Da0(parscpp["Da0"]);   // Dead
  const std::vector<double> oNa(parscpp["oNa"]);   // 1/number per age group - constant over time
...
//LATER: weekly by age
  ...
  for (int ia = 0; ia < na; ia++) { //age
    ig = ia + is*na;       
    Sig = Sa0[ig]; E1ig = E1a0[ig]; E2ig = E2a0[ig]; I1ig = I1a0[ig]; I2ig = I2a0[ig]; U1ig = U1a0[ig]; U2ig = U2a0[ig]; Rig = Ra0[ig]; Dig = Da0[ig];
  ...
  for (int it = 0; it < (nt-1); it++) {	//Crucial: nt-1 ////////////////////////
    ...
    for (int ia = 0; ia < na; ia++) { //////////////////////////////////////////
      ...
      for (int ia2 = 0; ia2 < na; ia2++) {
           ...
           FOI       += beta*ua*cmi*( I1_0[ig2] + I2_0[ig2] + f*U1_0[ig2] + f*U2_0[ig2] )*oNa[ig2]; //TODO:UPDATE oNA
      }} //ir, ib
